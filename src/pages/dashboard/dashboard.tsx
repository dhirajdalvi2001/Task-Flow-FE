...
export default function Dashboard() {
  const {data: dashboardCharts, isLoading} = useQuery<DashboardCharts>({
    ...
    queryFn: async () => {
      const response = await protectedAxiosInstance.get<DashboardCharts>("/charts/");
  ...
  const charts = useMemo(() => [
    {
      ...
      className: "bg-gradient-to-br from-orange-500/50 to-orange-500/30 via-80% via-orange-500/20",
      icon: <CircleXIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "Completed Tasks",
      value: dashboardCharts?.completed_tasks,
      className: "bg-gradient-to-br from-green-500/80 to-green-500/50 via-80% via-green-500/40",
      icon: <CircleCheckIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "In Progress Tasks",
      value: dashboardCharts?.in_progress_tasks,
      className: "bg-gradient-to-br from-yellow-500/50 to-yellow-500/30 via-80% via-yellow-500/20",
      icon: <CircleDashedIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "Pending Tasks",
      value: dashboardCharts?.pending_tasks,
      className: "bg-gradient-to-br from-blue-500/50 to-blue-500/30 via-80% via-blue-500/20",
      icon: <CircleDashedIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "Tasks Due in Three Days",
      value: dashboardCharts?.tasks_due_in_three_days,
      className: "bg-gradient-to-br from-purple-500/50 to-purple-500/30 via-80% via-purple-500/20",
      icon: <CircleDashedIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "Tasks Overdue",
      value: dashboardCharts?.tasks_overdue,
      className: "bg-gradient-to-br from-red-500/50 to-red-500/30 via-80% via-red-500/20",
      icon: <CircleXIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
    {
      title: "Total Tasks",
      value: dashboardCharts?.total_tasks,
      className: "bg-gradient-to-br from-gray-500/50 to-gray-500/30 via-80% via-gray-500/20",
      icon: <CircleDashedIcon className="size-6  sm:size-8 lg:size-10 text-white"/>,
    },
  ], [dashboardCharts]);
